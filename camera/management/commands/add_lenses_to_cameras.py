        ...
        for brand in CAMERA_BRANDS:
            for brand_series in list_brand_series(brand=brand):
                for brand_model in list_brand_series_models(brand=brand,brand_series=brand_series):
                    try:
                        cam = Camera.objects.get(brand=brand, brand_model=brand_model)
                    except Camera.DoesNotExist:
                        cam = None

                    if cam:
                        print("Looking up asins for " + cam.title)
                        asins = lookup_lens_asins_for_cam(brand=brand, brand_series=brand_series, brand_model=brand_model)
                        if asins:
                            for asin in asins:
                                print("Retreiving lens with asin " + asin + " from database...")
                                try:
                                ...
                                if lens:
                                    print("Adding lens " + lens.title + " to camera " + cam.title)
                                    cam.lenses.add(lens)
                                else:
                                    print("couldnt get lens from database, probably not available on api, skipping...")

                    else:
                        print("Unable to lookup camera..." + brand + " " + brand_series + " " + brand_model)
