...
class Command(BaseCommand):
    ...
    def handle(self, **options):
        ...
        for cam in soup.findAll("camera"):
            ...
            c, created = Camera.objects.get_or_create(title=cam_title, brand=CameraBrand.objects.get(title="Nikon"), megapixels=cam_megapixels, lcd_screen_size=cam_lcd_screen_size, memory_type=cam_memory_type, product_thumb_sm=cam_sm_thumb, product_thumb_lg=cam_lg_thumb)
            if created:
...
"""
            print("Camera Name: " + cam_title)
            print("Megapixels: " + cam_megapixels)
            print("LCD Screen Size: " + cam_lcd_screen_size)
            print("Memory type: " + cam_memory_type)
            print("Small thumbnail: " + cam_sm_thumb)
            print("Large thumbnail: " + cam_lg_thumb)
            print("------------------------------------------------------")
            print("")

        for cams in soup.findAll("camera"):
            for cam in cams.findAll("name"):
                cam_title = cam.text
                print(cam_title)
"""

#c, created = Camera.objects.get_or_create(title="Nikon D300", brand=CameraBrand.objects.get(title="Nikon"))
